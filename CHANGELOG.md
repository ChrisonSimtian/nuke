...
- **Renamed public type `MSBuildSettings` → `MsBuildSettings`** for casing consistency with the rest of the API surface (PascalCase acronym: `MsBuild`, not `MSBuild`). The MSBuild tool wrapper's settings type, its extension methods (`Fallout.Common.Tools.MSBuild.MsBuildSettingsExtensions`, `Fallout.Common.CI.TeamCity.MSBuildSettingsExtensions`), and `MSBuildTasks` all reference the new name.
  - **Migration / impact**: replace `MSBuildSettings` with `MsBuildSettings` in build code (e.g. `MSBuild(_ => _.Set…())` lambdas are unaffected — they infer the settings type; only explicit `MSBuildSettings`-typed locals/fields/configurators need updating). The `DotNetMSBuildSettings` type (the `dotnet msbuild` wrapper) is unrelated and **not** renamed.
  - **No shim**: this is a hard rename with no `[Obsolete]` alias — it rides `experimental` and is batched to the major cut rather than landing mid-year.
