...
>
> **Further amended by [ADR-0007](0007-cut-release-branch-on-demand.md) (2026-06-08).** The release branch is now **cut on demand at the first release** rather than being long-lived from year start, and ADR-0001's *"branches are cheap, don't delete"* stance is relaxed for dead branches with no unique history (tags are the durable release markers). The trigger, `validate-ref`, environments, and `prepare-release` mechanics are unchanged.
>
> **Further amended by [ADR-0009](0009-gitflow-and-semver-reversion.md) (2026-08-23).** The project moved back from ADR-0004's calendar versioning to classic semver, staying on `10.x` until a breaking change actually becomes necessary (v11 is deferred, not resumed immediately). It also adopted a literal GitFlow branch model: `main` is renamed to `develop`, taking over the integration-trunk/preview role, and the current `release/v10.4` branch is renamed to `main`, so **GA tags now fire from `main`** again instead of a `release/vN`-style branch. The release branch itself, the tag-triggered CD pipeline, and GitHub Environments described below stay the same — only the tag-source branch and the versioning scheme changed.
