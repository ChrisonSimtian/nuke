...
- **Style is codified in a committed `.editorconfig`** (plus `fallout.slnx.DotSettings` for Rider/ReSharper) — it's the source of truth for formatting, naming, and analyzer severities. The `build/` dogfood scripts get a relaxed `[build/**.cs]` section (explicit accessibility modifiers optional). Line endings are LF (matches `.gitattributes`); keep `dotnet format` runnable by not fighting it.
...
- Don't remove or bypass the committed `.editorconfig` / `fallout.slnx.DotSettings` — they're the style source of truth. Change a rule deliberately, not by deleting the file.
