...
- `main` — the **integration trunk *and* the sole prerelease lane.** Default branch. **Both** deliberate improvements + bug fixes **and** faster/AI-assisted work land here. Every push publishes an NB.GV-native prerelease `YYYY.MINOR.PATCH-preview.<height>.g<commit>` (currently `10.5.0-preview.<height>.g<commit>`, on the 10.x line) to **GitHub Packages only — never nuget.org.** Ordinary review.
