...
Merging to `main` publishes a **preview prerelease** (`2026.MINOR.PATCH-preview.…`) to **GitHub Packages only** — never nuget.org. This is the preview lane. **Stable releases** fire from `release/YYYY` branches via tag push, with a multi-channel publish fan-out (GitHub Packages + GitHub Releases by default; nuget.org is **opt-in**). The full lifecycle is documented in [docs/website/branching-and-release.md](docs/website/branching-and-release.md):
